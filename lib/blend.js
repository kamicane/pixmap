...
// basically a copy / paste from the SVG spec
...
// applies blendf to color2
function blend (color1, color2, blendf, amount = 1) {
  amount |= 0

  // small optimizations.

  if (amount === 0 || color1[3] === 0) return // no op, destination stays the same

  if (color2[3] === 0) { // copy color1 to color2 when destination alpha is 0
    color1.copy(color2)
    return
  }
